...
function initboxBubbleText()
{
	let pNodes1 = document.getElementById('box');
	pNodes1.innerHTML='\
        <p>冒泡排序算法的名字由来是因为越大的元素会经由交换“浮”到数列的顶端.<br><br>&nbsp稳定性:稳定<br>&nbsp时间复杂度O(n^2)<br>&nbsp空间复杂度:O(1)</p>\
        <span><img style="position: absolute; top: 35px;left: 7px" src="arrow_white_right.png" alt=""/></span>\
		';
}
...
function initboxmergeText()
{
	let pNodes1 = document.getElementById('box');
	pNodes1.innerHTML='\
        <p>归并排序可以说是将已有序的子序列合并，得到完全有序的序列.<br><br>&nbsp&nbsp稳定性:稳定<br>&nbsp&nbsp时间复杂度O(nlogn)<br>&nbsp&nbsp空间复杂度:O(n)</p>\
        <span><img style="position: absolute; top: 35px;left: 7px" src="arrow_white_right.png" alt=""/></span>\
		';
}
