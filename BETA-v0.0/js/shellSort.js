...
function shellSort2(resArr) {
	...
	for (var fraction = Math.floor(len / 2); fraction > 0; fraction = Math.floor(fraction / 2)) {
		this.pushArr(resArr.slice(), -1 , -1 , -1 , resArr.length-1 , 1 , 2);
		for (var i = fraction; i < len; i++) {
			for (var j = i - fraction; j >= 0 && resArr[j] > resArr[fraction + j]; j -= fraction) {
				this.pushArr(resArr.slice(), j , fraction + j , -1 , resArr.length-1 , 1 , 3);
				this.pushArr(resArr.slice(), j , fraction + j , -1 , resArr.length-1 , 2 , 4);
				var temp = resArr[j];
