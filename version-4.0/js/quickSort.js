...
function initQuickText(){
    ...
    pNodes.innerHTML='\
        ...
        <p>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbspfind max j fullfill A[i]<A[pivot]</p>\
...
function partition(resArr, left, right) {
    var pivot = resArr[Math.floor((right + left) / 2)],
        i = left,
        j = right;
    this.pushArr(resArr.slice(), pivot , -1 , left - 1 , right , 1 , 1);
    while (i <= j) {
        ...
        if (i <= j) {
            this.pushArr(resArr.slice(), i , j , left - 1 , right , 2 , 2);
            swap(resArr, i, j);
            i++;
            j--;
        }
